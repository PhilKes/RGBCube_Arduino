...
{  
   ...
   */
   for(int i=0;i<CUBE_SIZE;i++)
     ValueLed[i][FrameCount]=maxbright;
   if(FrameCount==107)
    FrameCount=0;
}
void Snake()
...
void AllRed()
{
    ...
    for(int l=0;l<CUBE_SIZE;l++)
    {
      for(int y=0;y<CUBE_SIZE;y++)
      {
        for(int x=0;x<CUBE_SIZE;x++)
        {
  	       ValueLed[l][x*CUBE_SIZE+y*CUBE_SIZE*3]=brightR;
        }
      }
    }
...
void AllGreen()
{
    ...
    for(int l=0;l<CUBE_SIZE;l++)
    {
      for(int y=0;y<CUBE_SIZE;y++)
      {
        for(int x=0;x<CUBE_SIZE;x++)
        {
  	ValueLed[l][x*CUBE_SIZE+1+y*CUBE_SIZE*3]=brightG;
        }
      }
    }
}
void AllBlue()
{
    for(int l=0;l<CUBE_SIZE;l++)
    {
      for(int y=0;y<CUBE_SIZE;y++)
      {
        for(int x=0;x<CUBE_SIZE;x++)
        {
  	ValueLed[l][x*CUBE_SIZE+1+y*CUBE_SIZE*3]=brightB;
        }
      }
    }
}
      ...
      for(int j=0;j<CUBE_SIZE;j++)
      {
          ...
          if(rand==0)
            //ValueLed[j][LedRed[y][x]]=maxbright;
             ValueLed[j][x*3+y*CUBE_SIZE*3]=maxbright;
          else if(rand==1)
            //ValueLed[j][LedGreen[y][x]]=maxbright;
            ValueLed[j][x*3+1+y*CUBE_SIZE*3]=maxbright;
          else if(rand==2)
            //ValueLed[j][LedBlue[y][x]]=maxbright;
            ValueLed[j][x*3+2+y*CUBE_SIZE*3]=maxbright;
  		    }
        }
...
void randomLeds(int anzR,int anzG,int anzB)
{
      ...
      {
        //ValueLed[random(0,CUBE_SIZE)][LedRed[random(0,CUBE_SIZE)][random(0,CUBE_SIZE)]]=maxbright;
        ValueLed[random(0,CUBE_SIZE)][random(0,CUBE_SIZE)*3+random(0,CUBE_SIZE)*CUBE_SIZE*3]=maxbright;
      }
      for (int p=anzG;p>0;p--)
      {
        //ValueLed[random(0,CUBE_SIZE)][LedGreen[random(0,CUBE_SIZE)][random(0,CUBE_SIZE)]]=maxbright;
        ValueLed[random(0,CUBE_SIZE)][random(0,CUBE_SIZE)*3+1+random(0,CUBE_SIZE)*CUBE_SIZE*3]=maxbright;
      }
      for (int p=anzB;p>0;p--)
      {
        //ValueLed[random(0,CUBE_SIZE)][LedBlue[random(0,CUBE_SIZE)][random(0,CUBE_SIZE)]]=maxbright;
        ValueLed[random(0,CUBE_SIZE)][random(0,CUBE_SIZE)*3+2+random(0,CUBE_SIZE)*CUBE_SIZE*3]=maxbright;
      }
      FrameCount=0;
